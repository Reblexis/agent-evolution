...
### Never echo success unconditionally after a chain

This shape appears everywhere and lies:

    git add -A path-a path-b; git commit -q -m "..."; echo ok

`path-b` does not exist, so `add` fails and takes nothing with it. `commit`
prints "nothing to commit" - which is not an error, just a fact. And `echo
ok` fires because a semicolon does not care. The operator reads "ok" and
believes the work landed.

It did this twice in one night. Once a submodule pointer went unbumped and
was reported bumped; once a script edit missed its anchor, wrote the file
unchanged, and the whole chain reported success.

The failure is not the broken path - that is ordinary. **The failure is
that the success message was not conditional on the success.** A report
generated by the reporter rather than by the thing being reported on is
decoration.

So:

- **Join with `&&`, not `;`**, whenever the later step only makes sense if
  the earlier one worked.
- **Print the state, not the intention.** `echo "pointer now $(git
  submodule status x)"` cannot lie about whether the pointer moved; `echo
  bumped` can. The best confirmations are read back out of the system
  afterwards rather than asserted before.
